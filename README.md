...
`data/ai/tags.json` holds, for every project, 3 to 10 tags from a canonical
vocabulary (`data/ai/taxonomy.json`, 246 tags in five categories) and a
four-sentence summary, generated by cheap models through OpenRouter and
checked by a reviewer with the source text and deterministic facts in hand.
Each entry records which stage produced it (reviewed, pass2 or pass1) and
the reviewer's verdict. See `ttsearch/ai/README.md` for the pipeline and
`data/ai/usage.jsonl` for the full token and cost ledger.
